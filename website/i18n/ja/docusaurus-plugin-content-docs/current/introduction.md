...
## 推奨フロー: Payment-Gated Provisioning

初回決済が完了するまでサービスを開始しないケースには、**Payment-Gated Provisioning**パターンを推奨します。既存の`Suspended`ステータスを統一的な「サービス非稼働」状態として再利用し、新規ステータスは不要です。
...
`Suspended`は「初回決済待ち」と「未払い停止」の両方で統一的な「サービス非稼働」状態として機能します。
