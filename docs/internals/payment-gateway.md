...
func (s *PaymentService) ProcessPayment(
    ...
    // 0. 境界検証（issue #241, BREAKING pre-1.0）: 空の IdempotencyKey は
    //    ErrCodeValidation で**入口で即時拒否**する（重複課金防止はキーに依存する。
    //    以前の「空キーは冪等性チェックをスキップして続行」という許容は廃止）
    if input.IdempotencyKey == "" { /* ErrCodeValidation を返す */ }
    ...
    // 3.5 ゲートウェイ顧客IDの解決（issue #231）: 請求書の AccountID を
    //     port.CustomerIDResolver（WithCustomerIDResolver で配線）で解決する。
    //     未配線なら AccountID をそのまま送る（legacy identity mapping、§3.3）。
    //     解決失敗はここで中断 — ゲートウェイに触れる前なので money-safe。
...
type ProcessPaymentInput struct {
    PaymentMethodID string          // 任意（空ならフォールバックチェーンで解決）
    PaymentMethod   payment.PaymentMethod // 任意（未設定なら ChargeResponse から解決）
    Amount          shared.Money
    Currency        shared.Currency
    IdempotencyKey  string          // 必須。空は ErrCodeValidation（issue #241）
    Metadata        map[string]string
}
