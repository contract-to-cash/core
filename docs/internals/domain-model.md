...
// 通貨不一致（両者非ゼロのとき）を拒否する。さらに未知の ContractType も拒否する
// （one_time / subscription / usage_based 以外は validation error。ContractType は下流の
// 課金分岐を駆動するため。検証はコマンド受付時のみ — Apply / リプレイは歴史的イベントを
// そのまま受け入れる）。
