...
| テナント分離 | **DB-per-tenant**（PostgreSQL） | core は設計上マルチテナントを利用者に委ねる（design-decisions 1.3）。物理分離が最も安全で、`adapters/postgres` をテナントごとに素直に使える |
...
1. フック実装は **テナント DB のアウトボックステーブルに行を書くだけ**
   （design-decisions 6.2 によりプラグインはコアと同一 tx → コミットと原子的に確定し、
